...
function varargout=polyreg(p,X,z,O=1)
  ...
  %% Extend the X designer no made it polynomial
  X = matrix_design(X, O);
  p = matrix_design(p, O);
  ...
  Xnormal = nx.fit_transform(X);
  pnormal = nx.transform(p);

  %% Normalize labels values
  nz = normalizer(str_normalizer);
  znormal = nz.fit_transform(z);

  %% Solve the polynomial regression
  theta=pinv(Xnormal)*znormal(:);

  %% Result without normalize
  rz= nz.itransform(pnormal*theta);
